...
tag_name = {}
spid = []
output_file = open(r'./output_files_Sprint3.txt','a+')
                ...
                if canInsert:  # Skip for the first time..so every time you end up with all the data for LEVEL 0 insert it in the table
                    ...
                    if flag == 1:
                        conn.execute("""INSERT INTO INDIVIDUAL VALUES (?,?,?,?,?,?,?,?,?);""", (
                            obj.id, obj.name, obj.sex, obj.birth, obj.age, obj.alive, obj.death, obj.child_id,
                            obj.spouse_id))
                        conn.commit()
                    elif flag == 2:
